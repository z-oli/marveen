...
EOF
  ...
  if { [ "$CODE" = "200" ] || [ "$CODE" = "201" ]; } && [ -n "$ID" ]; then
    ...
    #
    # A WARN ETTOL FUGGETLEN, MASODIK hiba-alak (2026-09-23, rebase-feloldas): a router
    # akkor is 200-at es id-t ad, ha a cimzett NEM fut, es ezt kulon `warning` mezoben
    # mondja meg -- olyankor az uzenet ELVESZETT, nem sorba allt. A ketto egymastol
    # fuggetlenul romolhat el (ep tartalom nem futo cimzettnek, csonka tartalom futonak),
    # ezert MINDKETTO kiirodik. A `bajt=` mezo helye valtozatlan, mert a CLAUDE.md erre
    # hivatkozik.
    BAJT="$(printf '%s' "$C" | wc -c | tr -d ' ')"
    if [ -n "${WARN:-}" ]; then
      echo "OK id=$ID bajt=$BAJT  WARNING: $WARN" >&2
      echo "OK id=$ID bajt=$BAJT (warning)"
    else
      echo "OK id=$ID bajt=$BAJT"
    fi
    exit 0
  fi
  sleep 1
